...
from d_mindformer.models.d_mindformer_out_trans_final_merge import DecisionMindformer
# from d_mindformer.models.d_mindformer_out_tran import DecisionMindformer
# from d_mindformer.models.d_mindformer_out_trans_merge_dosth import DecisionMindformer
...
def experiment(
            ...
            for _ in range(num_eval_episodes):
                with torch.no_grad():
                    if model_type == "dt":
                        ret, length = evaluate_episode_rtg(
                            env,
                            # env = gym.wrappers.RecordVideo(env, video_folder='', name_prefix='mario'),
                            state_dim,
                            act_dim,
                            model,
                            max_ep_len=max_ep_len,
    ...
    if log_to_wandb:
        ...
        wandb.init(
            name=exp_prefix,
            group=group_name,
            project="Inference-transformer-less-step",
            # project="Inference-transformer-5000-step",
            # project="Inference-transformer-2000-step-halfcheetah",
            config=variant,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="halfcheetah")  # walker2d  hopper   halfcheetah
    parser.add_argument(
    ...
    experiment("ACT-out-trans-finial-merge", variant=vars(args))
    # experiment("ACT-out-trans", variant=vars(args))
