...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        super().__init__()
        self.reward_scale = 1.0  # 奖励缩放因子，可根据实际情况调整
    ...
    def _attn(
        self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False, rewards=None
    ):
        w = torch.matmul(q, k)
        if self.scale:
        ...
        if rewards is not None:
            ...
            if rewards is not None:
                rewards = rewards.unsqueeze(1).unsqueeze(2)  # 奖励张量调整形状以匹配注意力分数张量

                w += rewards / self.reward_scale  # 奖励信息与注意力分数相加，缩放处理
        ...
        for i, (block, layer_past) in enumerate(
            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                outputs = block(
                    ...
                    use_cache=use_cache,
                    output_attentions=output_attentions,
                    rewards=rewards,  # 将奖励传递给每个块
                )

            hidden_states, present = outputs[
                :2
            ]  # 更新hidden_states并可能保存present用于后续计
            if use_cache is True:
